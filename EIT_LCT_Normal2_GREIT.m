function EIT_LCT_Normal2_GREIT()
%% Make folder and set filePath
    if ~exist('.\EIT_LCT_Normal2_GREIT','dir')
        mkdir('.\EIT_LCT_Normal2_GREIT');
    end
    EIT_GREIT_Filepath = '.\EIT_LCT_Normal2_GREIT';

    if ~exist('.\EIT_LCT_Normal2_GREIT_Voltage','dir')
        mkdir('.\EIT_LCT_Normal2_GREIT_Voltage');
    end
    EIT_V_Filepath = '.\EIT_LCT_Normal2_GREIT_Voltage';

    if ~exist('.\EIT_LCT_Normal2_GREIT_CP','dir')
        mkdir('.\EIT_LCT_Normal2_GREIT_CP');
    end
    EIT_CP_Filepath = '.\EIT_LCT_Normal2_GREIT_CP';

%% Get Contours
    thorax = shape_library('get','lct_normal2','boundary');
    rlung  = shape_library('get','lct_normal2','right_lung');
    llung  = shape_library('get','lct_normal2','left_lung');
    % one could also run:
...
%% Change the collapse area
    for collapseCase = 1:19%19:19
        ...
        switch collapseCase
            ...
            case 2
                collapseArea = inline('((x+0.34)/0.32).^2 + ((y+0.47)/0.16).^2 < 1','x','y','z');
            ...
            case 3
                collapseArea = inline('((x+0.34)/0.32).^2 + ((y+0.385)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 4
                collapseArea = inline('((x+0.34)/0.32).^2 + ((y+0.313)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 5
                collapseArea = inline('((x+0.34)/0.32).^2 + ((y+0.28)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);    
            ...
            case 6
                collapseArea = inline('((x+0.36)/0.4).^2 + ((y+0.27)/0.24).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 8
                collapseArea = inline('((x-0.35)/0.32).^2 + ((y+0.375)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 9
                collapseArea = inline('((x-0.4)/0.32).^2 + ((y+0.3)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 10
                collapseArea = inline('((x-0.4)/0.32).^2 + ((y+0.25)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 11
                collapseArea = inline('((x-0.5)/0.4).^2 + ((y+0.225)/0.23).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 12
                collapseArea = inline('(x/0.8).^2 + ((y+0.545)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 13
                collapseArea = inline('(x/0.8).^2 + ((y+0.495)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);   
            ...
            case 15
                collapseArea = inline('(x/0.8).^2 + ((y+0.41)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 16
                collapseArea = inline('(x/0.8).^2 + ((y+0.375)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 17
                collapseArea = inline('(x/0.8).^2 + ((y+0.34)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 18
                collapseArea = inline('(x/0.8).^2 + ((y+0.305)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
...
%% Run GREIT and save EIT data during sine TV wave
        for iter = 1:sigmaLen
            ...
            if iter == 1
                ...
                saveas(saveFig, ['LCT Normal2 Collapse shape (Collapse case ' num2str(collapseCase) ').bmp'])
                close(saveFig);
            ...
            imgPath = [EIT_GREIT_Filepath '\EIT_LCT_Normal2_GREIT_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.png'];
            imwrite(gridReconResult,imgPath,'PNG'); close;
            VPath = [EIT_V_Filepath '\EIT_LCT_Normal2_GREIT_Voltage_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(V',VPath);
            CPPath = [EIT_CP_Filepath '\EIT_LCT_Normal2_GREIT_CP_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(collapseP,CPPath);
            disp(['LCT Normal2 GREIT Case ' num2str(collapseCase) ' → ' num2str(iter) ' / ' num2str(sigmaLen) ' Finished ...']);
        end
    end
end
