function EIT_LCT_Normal4_FER()
%% Make folder and set filePath
    if ~exist('.\EIT_LCT_Normal4_FER','dir')
        mkdir('.\EIT_LCT_Normal4_FER');
    end
    EIT_FER_Filepath = '.\EIT_LCT_Normal4_FER';

    if ~exist('.\EIT_LCT_Normal4_FER_Voltage','dir')
        mkdir('.\EIT_LCT_Normal4_FER_Voltage');
    end
    EIT_V_Filepath = '.\EIT_LCT_Normal4_FER_Voltage';

    if ~exist('.\EIT_LCT_Normal4_FER_CP','dir')
        mkdir('.\EIT_LCT_Normal4_FER_CP');
    end
    EIT_CP_Filepath = '.\EIT_LCT_Normal4_FER_CP';
...
%% Run FER and save EIT data during sine TV wave
        for iter = 1:sigmaLen
            ...
            imgPath = [EIT_FER_Filepath '\EIT_LCT_Normal4_FER_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.png'];
            imwrite(gridReconResult,imgPath,'PNG'); close;
            VPath = [EIT_V_Filepath '\EIT_LCT_Normal4_FER_Voltage_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(V',VPath);
            CPPath = [EIT_CP_Filepath '\EIT_LCT_Normal4_FER_CP_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(collapseP,CPPath);
            disp(['LCT Normal4 FER Case ' num2str(collapseCase) ' → ' num2str(iter) ' / ' num2str(sigmaLen) ' Finished ...']);
        end
    end
end
