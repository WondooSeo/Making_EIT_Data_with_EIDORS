function EIT_EIDORS_Model_FER()
%% Make folder and set filePath
    if ~exist('.\EIT_EIDORS_Model_FER','dir')
        mkdir('.\EIT_EIDORS_Model_FER');
    end
    EIT_GREIT_Filepath = '.\EIT_EIDORS_Model_FER';

    if ~exist('.\EIT_EIDORS_Model_FER_Voltage','dir')
        mkdir('.\EIT_EIDORS_Model_FER_Voltage');
    end
    EIT_V_Filepath = '.\EIT_EIDORS_Model_FER_Voltage';
...
%% Run GREIT and save EIT data during sine TV wave
        for iter = 1:sigmaLen
            ...
            imgPath = [EIT_GREIT_Filepath '\EIT_EIDORS_Model_FER_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.png'];
            imwrite(gridReconResult,imgPath,'PNG'); close;
            VPath = [EIT_V_Filepath '\EIT_EIDORS_Model_FER_Voltage_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(V',VPath);
            CPPath = [EIT_CP_Filepath '\EIT_EIDORS_Model_FER_CP_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(collapseP,CPPath);
            disp(['EIDORS Model FER Case ' num2str(collapseCase) ' → ' num2str(iter) ' / ' num2str(sigmaLen) ' Finished ...']);
        end
    end
end
