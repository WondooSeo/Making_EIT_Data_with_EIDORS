function EIT_LCT_Obese1_FER()
%% Make folder and set filePath
    if ~exist('.\EIT_LCT_Obese1_FER','dir')
        mkdir('.\EIT_LCT_Obese1_FER');
    end
    EIT_GREIT_Filepath = '.\EIT_LCT_Obese1_FER';

    if ~exist('.\EIT_LCT_Obese1_FER_Voltage','dir')
        mkdir('.\EIT_LCT_Obese1_FER_Voltage');
    end
    EIT_V_Filepath = '.\EIT_LCT_Obese1_FER_Voltage';

    if ~exist('.\EIT_LCT_Obese1_FER_CP','dir')
        mkdir('.\EIT_LCT_Obese1_FER_CP');
    end
    EIT_CP_Filepath = '.\EIT_LCT_Obese1_FER_CP';

%% Get Contours
    thorax = shape_library('get','lct_obese1','boundary');
    rlung  = shape_library('get','lct_obese1','right_lung');
    llung  = shape_library('get','lct_obese1','left_lung');
    % one could also run:
...
%% Change the collapse area
    for collapseCase = 1:19%19:19
        ...
        switch collapseCase
            ...
            case 2
                collapseArea = inline('((x+0.35)/0.32).^2 + ((y+0.428)/0.16).^2 < 1','x','y','z');
            ...
            case 3
                collapseArea = inline('((x+0.35)/0.32).^2 + ((y+0.354)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 4
                collapseArea = inline('((x+0.35)/0.38).^2 + ((y+0.34)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 5
                collapseArea = inline('((x+0.35)/0.4).^2 + ((y+0.315)/0.23).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);    
            ...
            case 6
                collapseArea = inline('((x+0.35)/0.44).^2 + ((y+0.295)/0.26).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 7
                collapseArea = inline('((x-0.37)/0.32).^2 + ((y+0.39)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP); 
            ...
            case 8
                collapseArea = inline('((x-0.37)/0.32).^2 + ((y+0.27)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 9
                collapseArea = inline('((x-0.37)/0.32).^2 + ((y+0.193)/0.18).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 10
                collapseArea = inline('((x-0.37)/0.32).^2 + ((y+0.175)/0.25).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 11
                collapseArea = inline('((x-0.37)/0.34).^2 + ((y+0.14)/0.3).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 12
                collapseArea = inline('(x/0.8).^2 + ((y+0.48)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 14
                collapseArea = inline('(x/0.8).^2 + ((y+0.39)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 15
                collapseArea = inline('(x/0.8).^2 + ((y+0.355)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 16
                collapseArea = inline('(x/0.8).^2 + ((y+0.32)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 17
                collapseArea = inline('(x/0.8).^2 + ((y+0.32)/0.25).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 18
                collapseArea = inline('(x/0.8).^2 + ((y+0.31)/0.28).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 19
                collapseArea = inline('(x/0.8).^2 + ((y+0.27)/0.28).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
...
%% Run GREIT and save EIT data during sine TV wave
        for iter = 1:sigmaLen
            ...
            if iter == 1
                ...
                saveas(saveFig, ['LCT Obese1 Collapse shape (Collapse case ' num2str(collapseCase) ').bmp'])
                close(saveFig);
            ...
            imgPath = [EIT_GREIT_Filepath '\EIT_LCT_Obese1_FER_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.png'];
            imwrite(gridReconResult,imgPath,'PNG'); close;
            VPath = [EIT_V_Filepath '\EIT_LCT_Obese1_FER_Voltage_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(V',VPath);
            CPPath = [EIT_CP_Filepath '\EIT_LCT_Obese1_FER_CP_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(collapseP,CPPath);
            disp(['LCT Obese1 FER Case ' num2str(collapseCase) ' → ' num2str(iter) ' / ' num2str(sigmaLen) ' Finished ...']);
        end
    end
end
