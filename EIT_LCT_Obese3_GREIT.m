function EIT_LCT_Obese3_GREIT()
%% Make folder and set filePath
    if ~exist('.\EIT_LCT_Obese3_GREIT','dir')
        mkdir('.\EIT_LCT_Obese3_GREIT');
    end
    EIT_GREIT_Filepath = '.\EIT_LCT_Obese3_GREIT';

    if ~exist('.\EIT_LCT_Obese3_GREIT_Voltage','dir')
        mkdir('.\EIT_LCT_Obese3_GREIT_Voltage');
    end
    EIT_V_Filepath = '.\EIT_LCT_Obese3_GREIT_Voltage';

    if ~exist('.\EIT_LCT_Obese3_GREIT_GREIT_CP','dir')
        mkdir('.\EIT_LCT_Obese3_GREIT_GREIT_CP');
    end
    EIT_CP_Filepath = '.\EIT_LCT_Obese3_GREIT_GREIT_CP';
...
%% Run GREIT and save EIT data during sine TV wave
        for iter = 1:sigmaLen
            ...
            imgPath = [EIT_GREIT_Filepath '\EIT_LCT_Obese3_GREIT_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.png'];
            imwrite(gridReconResult,imgPath,'PNG'); close;
            VPath = [EIT_V_Filepath '\EIT_LCT_Obese3_GREIT_Voltage_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(V',VPath);
            CPPath = [EIT_CP_Filepath '\EIT_LCT_Obese3_GREIT_CP_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix([RCollapseP, LCollapseP],CPPath);
            disp(['LCT Obese3 GREIT Case ' num2str(collapseCase) ' → ' num2str(iter) ' / ' num2str(sigmaLen) ' Finished ...']);
        end
    end
end
