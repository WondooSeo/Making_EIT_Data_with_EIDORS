function EIT_LCT_Normal1_GREIT()
%% Make folder and set filePath
    if ~exist('.\EIT_LCT_Normal1_GREIT','dir')
        mkdir('.\EIT_LCT_Normal1_GREIT');
    end
    EIT_GREIT_Filepath = '.\EIT_LCT_Normal1_GREIT';

    if ~exist('.\EIT_LCT_Normal1_GREIT_Voltage','dir')
        mkdir('.\EIT_LCT_Normal1_GREIT_Voltage');
    end
    EIT_V_Filepath = '.\EIT_LCT_Normal1_GREIT_Voltage';

    if ~exist('.\EIT_LCT_Normal1_GREIT_CP','dir')
        mkdir('.\EIT_LCT_Normal1_GREIT_CP');
    end
    EIT_CP_Filepath = '.\EIT_LCT_Normal1_GREIT_CP';

%% Get Contours
    thorax = shape_library('get','lct_normal1','boundary');
    rlung  = shape_library('get','lct_normal1','right_lung');
    llung  = shape_library('get','lct_normal1','left_lung');
    % one could also run:
    ...
    shape = { 0,                % height
        ...
        [4,70],                 % perform smoothing with 50 points
...
%% Change the collapse area
    for collapseCase = 1:19%19:19
        ...
        switch collapseCase
            ...
            case 2
                collapseArea = inline('((x+0.34)/0.32).^2 + ((y+0.5)/0.16).^2 < 1','x','y','z');
            ...
            case 3
                collapseArea = inline('((x+0.34)/0.32).^2 + ((y+0.41)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 4
                collapseArea = inline('((x+0.34)/0.32).^2 + ((y+0.325)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 5
                collapseArea = inline('((x+0.35)/0.32).^2 + ((y+0.24)/0.23).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);    
            ...
            case 6
                collapseArea = inline('((x+0.36)/0.35).^2 + ((y+0.18)/0.27).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 7
                collapseArea = inline('((x-0.34)/0.32).^2 + ((y+0.53)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP); 
            ...
            case 8
                collapseArea = inline('((x-0.34)/0.32).^2 + ((y+0.45)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 9
                collapseArea = inline('((x-0.38)/0.32).^2 + ((y+0.38)/0.16).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 10
                collapseArea = inline('((x-0.4)/0.5).^2 + ((y+0.357)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 11
                collapseArea = inline('((x-0.45)/0.5).^2 + ((y+0.327)/0.25).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 12
                collapseArea = inline('(x/0.8).^2 + ((y+0.597)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 14
                collapseArea = inline('(x/0.8).^2 + ((y+0.507)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 15
                collapseArea = inline('(x/0.8).^2 + ((y+0.464)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 16
                collapseArea = inline('(x/0.8).^2 + ((y+0.43)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 18
                collapseArea = inline('(x/0.8).^2 + ((y+0.36)/0.2).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
            ...
            case 19
                collapseArea = inline('(x/0.8).^2 + ((y+0.34)/0.22).^2 < 1','x','y','z');
                [row, ~] = find(elem_select(tempImg.fwd_model, collapseArea));
                targetLungElem = setdiff(lungElem,row);
                collapseP = 1 - length(targetLungElem) / length(lungElem);
                disp(collapseP);
...
%% Run GREIT and save EIT data during sine TV wave
        for iter = 1:sigmaLen
            ...
            if iter == 1
                ...
                saveas(saveFig, ['LCT Normal1 Collapse shape (Collapse case ' num2str(collapseCase) ').bmp'])
                close(saveFig);
            ...
            imgPath = [EIT_GREIT_Filepath '\EIT_LCT_Normal1_GREIT_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.png'];
            imwrite(gridReconResult,imgPath,'PNG'); close;
            VPath = [EIT_V_Filepath '\EIT_LCT_Normal1_GREIT_Voltage_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix(V',VPath);
            CPPath = [EIT_CP_Filepath '\EIT_LCT_Normal1_GREIT_CP_collapse_case_' num2str(collapseCase) '_' num2str(iter) '.csv'];
            writematrix([RCollapseP, LCollapseP],CPPath);
            disp(['LCT Normal1 GREIT Case ' num2str(collapseCase) ' → ' num2str(iter) ' / ' num2str(sigmaLen) ' Finished ...']);
        end
    end
end
