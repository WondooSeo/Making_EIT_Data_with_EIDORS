...
%% Get Contours
    ...
    rlung  = shape_library('get','lct_normal3','right_lung');
    llung  = shape_library('get','lct_normal3','left_lung');
    % one could also run:
...
%% Run FER and save EIT data during sine TV wave
        for iter = 1:sigmaLen
            ...
            if iter == 1
                ...
                saveas(saveFig, ['LCT Normal3 Collapse shape (Collapse case ' num2str(collapseCase) ').bmp'])
                close(saveFig);
